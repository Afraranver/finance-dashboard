...
import { View, Text, ScrollView, ActivityIndicator, Dimensions, Platform } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useAnalytics, useCategories } from '@/services/api';
import { CategoryChart } from '@/components/CategoryChart';

// Conditional imports
const VictoryBar = Platform.OS === 'web' 
  ? require('victory').VictoryBar 
  : require('victory-native').VictoryBar;
const VictoryChart = Platform.OS === 'web' 
  ? require('victory').VictoryChart 
  : require('victory-native').VictoryChart;
const VictoryAxis = Platform.OS === 'web' 
  ? require('victory').VictoryAxis 
  : require('victory-native').VictoryAxis;
const VictoryTheme = Platform.OS === 'web' 
  ? require('victory').VictoryTheme 
  : require('victory-native').VictoryTheme;
const VictoryGroup = Platform.OS === 'web' 
  ? require('victory').VictoryGroup 
  : require('victory-native').VictoryGroup;
...
export default function AnalyticsScreen() {
  ...
  const chartWidth = Platform.OS === 'web' ? 600 : width - 64;

  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={['bottom']}>
      <ScrollView className="flex-1">
        <View className={`py-6 ${Platform.OS === 'web' ? 'px-8' : 'px-4'}`}>
          {/* Income vs Expenses Chart */}
          <View className="bg-white rounded-2xl p-4 mb-4 shadow-sm border border-gray-100">
            <Text className="text-lg font-bold text-gray-900 mb-4">Income vs Expenses</Text>
            <View className={Platform.OS === 'web' ? 'items-center' : ''}>
              <VictoryChart
